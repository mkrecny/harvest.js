  ...
  self.dao.getIntersectionSize(this.dao.getUserPropertyKey(focus, self.subject), this.dao.getUserPropertyKey(comp, self.subject), function(err, sintersize){
   ...
   self.dao.sunionScard(focus_key, comp_key, function(err, sunionsize){
     ...
     if (significance > self.threshold){
       ...
       self.trigger('harvest:complete', focus, comp, 0);
     }
   });
  });
};
 ...
 * //TODO: make 'all' relative to subect eg: u:123:likes:all
 */
Harvest.prototype.harvestFromUser = function(focus, comp, significance){ 
  ...
  this.dao.srandSdiff(this.dao.getUserPropertyKey(focus, 'all'), this.dao.getUserPropertyKey(comp, self.subject), numElsToHarvest, function(err, harvest){
    if (harvest.length===0){
      ...
      return self.trigger('harvest:complete', focus, comp, 0);
    }
...
/*
 * Get all members of users subject set
 * @param user : varchar : user id  
 */
Harvest.prototype.getUserSubjectSet = function(user, callback){
  return this.dao.getUserSet(user, this.subject, callback);  
};

/*
 * Get the users harvest set
 * @param user : varchar ; user_id
 */
Harvest.prototype.getUserHarvest = function(user, callback){
  return this.dao.getUserSet(user, 'harvest', callback);
};
