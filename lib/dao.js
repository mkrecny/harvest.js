...
DAO.prototype.getIntersectionSize = function(a_key, b_key, callback){
  var script = 'return table.getn(redis.call("sinter", KEYS[1], KEYS[2]))';
...
DAO.prototype.srandSdiff = function(a_key, b_key, num_rand_els, callback){
  var script = 'local keys,argv,rcall = KEYS,ARGV,redis.call; local diff = rcall("sdiff", keys[1], keys[2]); local max = tonumber(argv[1]); if max > table.getn(diff) then max = table.getn(diff) end local output = {}; for i=0, max-1 do table.insert(output, table.remove(diff)) end return output';
...
DAO.prototype.srandSubStore = function(src_key, dst_key, subset_size, callback){
  var script = 'local keys,argv,rcall = KEYS,ARGV,redis.call; for i=0, tonumber(argv[1])-1 do rcall("sadd", keys[2], rcall("srandmember", keys[1])) end return {ok="OK"}';
...
DAO.prototype.sunionScard = function(a_key, b_key, callback){
  var script = 'return table.getn(redis.call("sunion", KEYS[1], KEYS[2]))';
  return this.redis.eval(script, 2, a_key, b_key, callback);
};
